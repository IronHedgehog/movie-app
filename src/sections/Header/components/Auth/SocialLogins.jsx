...
export default function SocialLogins({ onSuccess }) {
  const dispatch = useDispatch();

  // Універсальна функція для обробки логіну через провайдерів
  const handleProviderLogin = async (providerAction) => {
    const result = await dispatch(providerAction());
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      ...
      <button
        ...
        type="button"
        className="flex justify-center items-center py-2.5 border border-zinc-700 rounded-lg hover:bg-zinc-800 transition-colors"
      >
      ...
      <button
        ...
        disabled
        title="Coming soon"
        className="flex justify-center items-center py-2.5 border border-zinc-700 rounded-lg opacity-50 cursor-not-allowed transition-colors"
      >
        <svg
      ...
      {/* 3. РОБОЧА КНОПКА GitHub (Тепер зі справжнім кото-восьминогом) */}
      <button
